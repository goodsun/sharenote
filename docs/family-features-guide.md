...
共有手帳（shareNOTE）の家族共有機能は、メモを家族メンバー間で簡単に共有できる機能です。招待コードを使った簡単な参加と、日本の家族制度を反映した用語を採用しています。
...
- または全メンバーが退出して最後の 1 人になれば自動的に個人状態に戻ります

## 🔄 メニューの動的表示

共有手帳（shareNOTE）のメニューは、ユーザーの状態に応じて自動的に表示項目が変わります：
...
A: コードの有効期限（5 分）が切れている可能性があります。新しいコードを生成してもらってください。
...
### Q: Alexa で作ったメモが Web に表示されない

A: 同じ Google アカウントでログインしているか確認してください。Alexa 側のアカウントリンクも確認が必要です。
...
_このガイドは家族共有機能の使い方を説明しています。技術的な詳細については[family-concept-verification.md](family-concept-verification.md)を参照してください。_
